...
pub fn prepare_kernel_page(ctx: InitializationContext<Stage2>) -> InitializationContext<Stage3> {
    ...
    let kernel_pages_table = system_table
        .boot_services()
        .allocate_pages(AllocateType::AnyPages, MemoryType::LOADER_DATA, mem_map_pages)
    ...
    unsafe {
        kernel_table.mapper_with_allocator(&mut kernel_page_allocator).identity_map_by_size(
    ...
    // Do a recursive map
    kernel_table.p4_mut()[511] =
        Entry(p4_frame.start_address().as_u64() | (EntryFlags::PRESENT | EntryFlags::WRITABLE).bits());
