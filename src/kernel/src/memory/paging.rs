...
use pager::{
    ...
    paging::{
        ActivePageTable, InactivePageTable, TableManipulationContext, Transferable,
        mapper::Mapper,
    ...
    A: FrameAllocator,
{
    ...
    new_table.transfer(&active_table, ctx.context_mut().boot_bridge.loaded_kernel(), allocator, false);

    for usable in ctx.context().boot_bridge().memory_map().entries().filter(|e| e.ty == MemoryType::CONVENTIONAL) {
        let virt = (KERNEL_DIRECT_PHYSICAL_MAP.as_u64() + usable.phys_start.as_u64()).into();
