...
// since sysretq adds +16 this is just a dummy
def_local!(pub static USER_CODE_SEG_DUMMY: pager::registers::SegmentSelector);
...
pub fn init_gdt(ctx: &mut InitializationContext<Stage3>) {
    ...
    let gdt_initializer = |cpu: &mut CpuLocalBuilder, ctx: &ApInitializationContext, id| {
        ...
        let user_code_dummy = gdt.add_entry(Descriptor::code_segment(), PrivilegeLevel::Ring3);
        ...
        log!(Debug, "Kernel code seg: {kernel_code_selector:?} Kernel data seg: {kernel_data_seg:?}");
        log!(Debug, "User code seg: {user_code_selector:?} User data seg: {user_data_seg:?}");
        unsafe {
        ...
        local_builder!(
            ...
            USER_CODE_SEG_DUMMY(user_code_dummy),
