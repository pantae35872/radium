...
impl Vga {
    pub fn new(ctx: &mut InitializationContext<Stage1>) -> Self {
        Self {
            ...
            backbuffer_address: unsafe {
                VirtAddr::new(alloc(Layout::from_size_align(0xFA00, align_of::<u8>()).expect("Layout not valid")) as u64)
            },
        }
    }
    ...
    unsafe fn write_registers(&mut self, registers_slice: &mut [u8]) {
        unsafe {
            ...
            for i in 0..5 {
                ...
                self.sequencer_data_port.write(*registers.next().unwrap_or(&0));
            }
            ...
            for i in 0..9 {
                ...
                self.graphic_controller_data_port.write(*registers.next().unwrap_or(&0));
            }

            for i in 0..21 {
                self.attribute_controller_reset_port.read();
                self.attribute_controller_index_port.write(i);
                self.attribute_controller_write_port.write(*registers.next().unwrap_or(&0));
            }
    ...
    pub fn set_mode(&mut self, width: u32, height: u32, colordepth: u32) -> bool {
        ...
        let mut g_320x200x256: [u8; 61] = [
            ...
            0x5F, 0x4F, 0x50, 0x82, 0x54, 0x80, 0xBF, 0x1F, 0x00, 0x41, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x9C, 0x0E,
            0x8F, 0x28, 0x40, 0x96, 0xB9, 0xA3, 0xFF, /* GC */
            0x00, 0x00, 0x00, 0x00, 0x00, 0x40, 0x05, 0x0F, 0xFF, /* AC */
            0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E, 0x0F, 0x41, 0x00,
            0x0F, 0x00, 0x00,
        ];
    ...
    pub fn put_pixel(&mut self, x: usize, y: usize, color_index: u8) {
        ...
        unsafe {
            write_volatile((self.get_frame_buffer_segment() + 320 * y + x) as *mut u8, color_index);
        }
    }

    pub fn swap(&self) {
        unsafe {
            ptr::copy_nonoverlapping(self.backbuffer_address.as_ptr::<u8>(), 0xA0000 as *mut u8, 0xFA00);
        }
    }
