...
use core::mem::{align_of, size_of, size_of_val, transmute, ManuallyDrop};
use core::ops::Add;
...
use crate::driver::pci::{self, PCIControler};
use crate::memory::paging::{Page, PhysicalAddress};
use crate::memory::{AreaFrameAllocator, Frame};
use crate::utils::oserror::OSError;
use crate::utils::port::{self, Port32Bit};
use crate::utils::VolatileCell;
use crate::{get_physical, inline_if, print, println, EntryFlags, ACTIVE_TABLE};
...
pub fn init(frame_allocator: &mut AreaFrameAllocator) {
    ...
    if let Some(abar_address) = abar_address {
            ...
            ))
        {
            ACTIVE_TABLE
                .get()
                .expect("Active table is not initialize")
                .lock()
                .map_to(
                    page,
                    frame,
